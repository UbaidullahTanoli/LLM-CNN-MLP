...
The model didn't overfit as the performance metrics kept on improving. This model performs exceptionally well compared to using a textual LLM to read reports or a CNN to study the images, as shown in the table above. The key features of the LLM+CNN+MLP model are as follows:
